...
function Convert-ToUtcIso([datetime]$Value) {
...
function New-Envelope(
    ...
    [ordered]@{
        state = $State
        quality = $Quality
        observed_at_utc = Convert-ToUtcIso $now
        fresh_until_utc = Convert-ToUtcIso $now.AddSeconds($FreshnessSeconds)
        source = $Source
        cadence_class = $SignalCadence
        reason = $Reason
        data = $Data
    }
}
...
function Invoke-BoundedProbe(
    ...
    if ($null -ne $ProbeOverrides -and $ProbeOverrides.ContainsKey($Name)) {
        $Body = [scriptblock]$ProbeOverrides[$Name]
    ...
    try {
        ...
        if (-not $async.AsyncWaitHandle.WaitOne([timespan]::FromSeconds($ProbeTimeoutSeconds))) {
            try { $ps.Stop() } catch { }
            return New-Envelope $Source $SignalCadence $null 'UNAVAILABLE' 'UNKNOWN' 'PROBE_TIMEOUT'
        }

        try {
            $result = @($ps.EndInvoke($async))
            if ($ps.HadErrors) {
                $message = (($ps.Streams.Error | ForEach-Object { $_.Exception.Message }) -join '; ')
                if ([string]::IsNullOrWhiteSpace($message)) { $message = 'PROBE_ERROR' }
                return New-Envelope $Source $SignalCadence $null 'UNAVAILABLE' 'UNKNOWN' (($message -replace '[\r\n]+',' ').Trim())
            }
            $data = if ($result.Count -eq 1) { $result[0] } else { @($result) }
            return New-Envelope $Source $SignalCadence $data
        }
        catch {
            return New-Envelope $Source $SignalCadence $null 'UNAVAILABLE' 'UNKNOWN' (($_.Exception.Message -replace '[\r\n]+',' ').Trim())
        }
    }
    catch {
        return New-Envelope $Source $SignalCadence $null 'UNAVAILABLE' 'UNKNOWN' (($_.Exception.Message -replace '[\r\n]+',' ').Trim())
...
try {
    ...
    if ($CadenceClass -in @('fast','all')) {
        $signals.cpu = Invoke-BoundedProbe 'cpu' 'Win32_Processor' 'FAST' {
        ...
        $signals.memory = Invoke-BoundedProbe 'memory' 'Win32_OperatingSystem' 'FAST' {
        ...
        $signals.processes = Invoke-BoundedProbe 'processes' 'Windows process table' 'FAST' {
        ...
        $signals.plugin_heartbeat = Invoke-BoundedProbe 'plugin_heartbeat' 'NINA telemetry projection metadata' 'FAST' {
            ...
            [ordered]@{ projection_path=$p; exists=($null -ne $f); last_write_at_utc=if($f){$f.LastWriteTimeUtc.ToString('yyyy-MM-ddTHH:mm:ss.fffZ')}else{$null}; plugin_version=$null }
        }
    }

    if ($CadenceClass -in @('medium','all')) {
        $signals.storage = Invoke-BoundedProbe 'storage' 'Win32_LogicalDisk/Get-PhysicalDisk' 'MEDIUM' {
    ...
    if ($CadenceClass -in @('slow','all')) {
        ...
        $signals.event_log = Invoke-BoundedProbe 'event_log' 'Windows Event Log bounded query' 'SLOW_ON_CHANGE' { $end=[datetime]::Now; $start=$end.AddHours(-1); $events=@(Get-WinEvent -FilterHashtable @{LogName=@('System','Application'); StartTime=$start; Level=@(1,2)} -ErrorAction SilentlyContinue | Select-Object -First 50 | ForEach-Object { [ordered]@{ log_name=$_.LogName; time_created_utc=$_.TimeCreated.ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ss.fffZ'); event_id=[int]$_.Id; level=$_.LevelDisplayName; provider=$_.ProviderName } }); [ordered]@{ window_start_utc=$start.ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ss.fffZ'); window_end_utc=$end.ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ss.fffZ'); events=$events } }
    ...
    $projection=[ordered]@{
        ...
        summary=[ordered]@{state='UNKNOWN';reasons=@([ordered]@{code='POLICY_NOT_ACTIVATED';signal=$null;severity=$null;evidence=$null})}
        cadence_class=(Get-GovernedCadence $CadenceClass)
        signals=$signals
        diagnostics=[ordered]@{collector_mode='READ_ONLY';safety_authority='OUTSIDE_SCOPE';automatic_remediation=$false;probe_timeout_seconds=$ProbeTimeoutSeconds;non_overlap='MUTEX_SKIP'}
    }

    $dir=Split-Path -Parent $OutputPath
    if(-not (Test-Path -LiteralPath $dir)){ New-Item -ItemType Directory -Path $dir -Force | Out-Null }
    $tmp=Join-Path $dir ('.'+[IO.Path]::GetFileName($OutputPath)+'.'+[guid]::NewGuid().ToString('N')+'.tmp')
    $backup=Join-Path $dir ('.'+[IO.Path]::GetFileName($OutputPath)+'.'+[guid]::NewGuid().ToString('N')+'.bak')
    try {
        $json=$projection | ConvertTo-Json -Depth 12
        Assert-Projection $json
        [IO.File]::WriteAllText($tmp,$json,(New-Object Text.UTF8Encoding($false)))
        Assert-Projection ([IO.File]::ReadAllText($tmp))

        if(Test-Path -LiteralPath $OutputPath -PathType Leaf){
            [IO.File]::Replace($tmp,$OutputPath,$backup,$true)
            if(Test-Path -LiteralPath $backup){ Remove-Item -LiteralPath $backup -Force -ErrorAction SilentlyContinue }
        } else {
            Move-Item -LiteralPath $tmp -Destination $OutputPath
        }
    }
    finally {
