...
const parseCsvLine=(line)=>{const rows=parseCsv(`frame\n${line}\n`);return rows.length?Object.values(rows[0]):[];};
...
const percentile=(values,p)=>{if(!values.length)return null;const sorted=[...values].sort((a,b)=>a-b);const i=Math.min(sorted.length-1,Math.max(0,Math.ceil((p/100)*sorted.length)-1));return sorted[i];};

const readWeatherDetail=async(metricsPath)=>{
  if(!metricsPath)return null;
  const sessionRoot=dirname(dirname(metricsPath));
  const weatherDir=join(sessionRoot,'raw','weather');
  try{
    const files=(await readdir(weatherDir)).filter(name=>name.toLowerCase().endsWith('.csv')).sort();
    if(!files.length)return null;
    const weatherPath=join(weatherDir,files[0]);
    const rows=parseCsv(await readFile(weatherPath,'utf8'));
    if(!rows.length)return null;
    const samples=downsample(rows,48).map(row=>({
...
const parseGuideLog=(text,sourcePath)=>{
  const samples=[];let segment=0;let settling=false;let active=false;
  for(const line of text.split(/\r?\n/)){
    if(line.startsWith('Guiding Begins at ')){segment+=1;settling=false;active=true;continue;}
    const low=line.toLowerCase();
    if(low.includes('settling started')){settling=true;continue;}
    if(low.includes('settling complete')){settling=false;continue;}
    if(!active||settling||!/^\d+,\s*[\d.]+,"[^"]+",/.test(line))continue;
    try{
      ...
      samples.push({segment,elapsedSeconds:round(elapsedSeconds,3),raArcsec:round(raArcsec,3),decArcsec:round(decArcsec,3),totalArcsec:round(Math.hypot(raArcsec,decArcsec),3)});
    }catch{}
  }
  if(!samples.length)return null;
  const ra=samples.map(s=>s.raArcsec),dec=samples.map(s=>s.decArcsec),total=samples.map(s=>s.totalArcsec);
  return {
    sourcePath:sourcePath.replaceAll('\\','/'),
    sampleCount:samples.length,
    segmentCount:Math.max(...samples.map(s=>s.segment)),
    rmsRaArcsec:round(rms(ra),3),
    rmsDecArcsec:round(rms(dec),3),
    rmsTotalArcsec:round(Math.hypot(rms(ra)??0,rms(dec)??0),3),
    p95TotalArcsec:round(percentile(total,95),3),
    maxTotalArcsec:round(Math.max(...total),3),
    samples:downsample(samples,96)
  };
};
...
const readGuidingDetail=async(metricsPath)=>{
  ...
  try{
    ...
    for(const name of files){
      const path=join(phd2Dir,name);
      const detail=parseGuideLog(await readFile(path,'utf8'),path);
...
const guidingProjection=(metrics,detail)=>{
  const guide=metrics?.phd2;
  const sampleCount=numberOrNull(guide?.guide_samples_valid)??detail?.sampleCount??null;
  return {
    state:sampleCount&&sampleCount>0?'AVAILABLE':'NOT_AVAILABLE',
    sampleCount,
    segmentCount:numberOrNull(guide?.guide_segments)??detail?.segmentCount??null,
    rmsRaArcsec:numberOrNull(guide?.rms_ra_arcsec)??detail?.rmsRaArcsec??null,
    rmsDecArcsec:numberOrNull(guide?.rms_dec_arcsec)??detail?.rmsDecArcsec??null,
    rmsTotalArcsec:numberOrNull(guide?.rms_total_arcsec)??detail?.rmsTotalArcsec??null,
    lostStarEvents:numberOrNull(guide?.lost_star_events),
    pulseGuideFailures:numberOrNull(guide?.pulse_guide_failures),
    scopeNote:'Proiezione scientifica storica derivata dal PHD2 GuideLog. Usa gli stessi campioni validi dell analytics (settling escluso, ErrorCode=0) e non rappresenta telemetria operativa realtime.',
    detail
  };
};

const buildCatalog=(sessions,targets,metadata,metricsBySession,weatherDetailBySession,guidingDetailBySession)=>{
...
const main=async()=>{
  ...
  const sessions=parseCsv(sessionsText);const metricsBySession=new Map();const weatherDetailBySession=new Map();const guidingDetailBySession=new Map();
  await Promise.all(sessions.map(async(session)=>{const sessionId=String(session.session_id||'').trim();const path=valueOrNull(session.source_metrics_path);if(!sessionId||!path)return;try{metricsBySession.set(sessionId,JSON.parse(await readFile(path,'utf8')));}catch{metricsBySession.set(sessionId,null);}const [weather,guiding]=await Promise.all([readWeatherDetail(path),readGuidingDetail(path)]);weatherDetailBySession.set(sessionId,weather);guidingDetailBySession.set(sessionId,guiding);}));
  const generated=stableJson(buildCatalog(sessions,parseCsv(targetsText),parseCsv(metadataText),metricsBySession,weatherDetailBySession,guidingDetailBySession));
