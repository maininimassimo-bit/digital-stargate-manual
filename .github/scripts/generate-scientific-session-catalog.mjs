...
const weatherProjection=(metrics)=>{
  const weather=metrics?.weather;
  if(!weather||typeof weather!=='object')return {state:'NOT_AVAILABLE',rowsTotal:null,rowsUnsafe:null,unsafePct:null,safePct:null,safeTransitions:null,scopeNote:null};
  ...
  const safePct=unsafePct===null?null:Number((100-unsafePct).toFixed(2));
  return {state:rowsTotal&&rowsTotal>0?'AVAILABLE':'NOT_AVAILABLE',rowsTotal,rowsUnsafe,unsafePct,safePct,safeTransitions:numberOrNull(weather.weather_safe_transitions_full_window),scopeNote:valueOrNull(weather.weather_scope_note)};
};
...
const main=async()=>{
  ...
  const sessions=parseCsv(sessionsText); const metricsBySession=new Map();
  await Promise.all(sessions.map(async(session)=>{const sessionId=String(session.session_id||'').trim();const path=valueOrNull(session.source_metrics_path);if(!sessionId||!path)return;try{metricsBySession.set(sessionId,JSON.parse(await readFile(path,'utf8')));}catch{metricsBySession.set(sessionId,null);}}));
  const generated=stableJson(buildCatalog(sessions,parseCsv(targetsText),parseCsv(metadataText),metricsBySession));
