...
export function buildAnomalyTrendReadModel(replay) {
  ...
  return {
    schema_version: '1.0',
    component: 'DSG.AnomalyTrendCenter.F4ReadModel',
    authority: 'projection',
    action_authority: 'NONE',
    baseline_commit: F3B_ACCEPTED_MERGE,
    upstream_component: projection.component,
    upstream_method_version: projection.method_version,
    source_component: projection.source_component,
    source_baseline_commit: projection.source_baseline_commit,
    session_id: projection.session_id,
    interaction_mode: 'READ_ONLY',
    advisory_mode: 'DESCRIPTIVE_ONLY',
    safety_authority: 'UNCHANGED_LOCAL_AUTHORITY',
    command_actions: [],
    summary: {
      total_records: projection.records.length,
      observations: count('OBSERVATION'),
      trend_measurements: count('TREND_MEASUREMENT'),
      anomaly_candidates: count('ANOMALY_CANDIDATE'),
      correlation_candidates: count('CORRELATION_CANDIDATE'),
      recommendations: count('RECOMMENDATION')
    },
    source_drilldown: {
      enabled: true,
      fields: ['source_record_refs', 'citation_refs', 'provenance_refs']
    },
    records: projection.records,
    limitations: [
      'HISTORICAL_PROJECTION_NOT_CURRENT_SAFETY_STATE',
      'NO_ANOMALY_THRESHOLD_AUTHORIZED',
      'CORRELATION_DOES_NOT_IMPLY_CAUSATION',
      'NO_COMMAND_OR_REMEDIATION_AUTHORITY',
      'BKL030_EAGLE_HISTORY_NOT_ONBOARDED'
    ]
  };
}
...
if (process.argv.includes('--write')) {
  ...
  if (!fs.existsSync(outputPath) || fs.readFileSync(outputPath, 'utf8').replace(/^\uFEFF/, '') !== serialized) {
