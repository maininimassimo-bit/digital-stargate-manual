...
export function discoverFromTexts({metadataText,exposureText,summaryText}){
  ...
  for(const meta of [...metadata].sort((a,b)=>String(a.session_id).localeCompare(String(b.session_id)))){
    ...
    for(const [filter_name,group0] of [...byFilter.entries()].sort(([a],[b])=>a.localeCompare(b))){
      ...
      p.citation_refs=[INPUTS.exposures,INPUTS.summary,INPUTS.metadata];
...
if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)){
  ...
  if(process.argv.includes('--json')) process.stdout.write(`${JSON.stringify(result,null,2)}\n`);
    ...
    console.log(`BKL-039 F5-A discovery OK: ${result.eligible_populations.length} eligible population(s), ${result.exclusions.length} exclusion(s).`);
