...
function localToUtc(local, zone) {
  ...
  const naive = Date.UTC(+m[1],+m[2]-1,+m[3],+m[4],+m[5],+m[6],+frac);
  let guess = naive;
  for (let i=0;i<3;i++) {
    const p=partsAt(guess,zone);
    const represented=Date.UTC(p.year,p.month-1,p.day,p.hour,p.minute,p.second,0);
    guess -= represented - Date.UTC(+m[1],+m[2]-1,+m[3],+m[4],+m[5],+m[6],0);
  }
...
].sort((a,b)=>Date.parse(a.event_time_utc)-Date.parse(b.event_time_utc)||a.source_order-b.source_order||a.replay_event_id.localeCompare(b.replay_event_id));

const citations = [
  citation('CIT-RPL-F3-CW-FIRST','CloudWatcher CSV historical evidence',cwRel,'/row/2'),
  citation('CIT-RPL-F3-NINA-DOME-OPEN','N.I.N.A. raw log evidence',ninaRel,'/line/contains:Opening dome shutter'),
  citation('CIT-RPL-F3-PHD2-GUIDING-BEGIN','PHD2 GuideLog historical evidence',phdRel,'/line/contains:Guiding Begins at')
];
const provenanceRecords = [
  provenance('PRV-RPL-F3-CW-FIRST','BKL040-F3-CLOUDWATCHER-LOCAL-TO-UTC-1',`${cwRel}#/row/2`,'RPL-20260815-CLOUDWATCHER-FIRST','CIT-RPL-F3-CW-FIRST'),
  provenance('PRV-RPL-F3-NINA-DOME-OPEN','BKL040-F3-NINA-LOCAL-TO-UTC-1',`${ninaRel}#/line/contains:Opening dome shutter`,'RPL-20260815-NINA-DOME-OPEN','CIT-RPL-F3-NINA-DOME-OPEN'),
  provenance('PRV-RPL-F3-PHD2-GUIDING-BEGIN','BKL040-F3-PHD2-GUIDE-ANCHOR-1',`${phdRel}#/line/contains:Guiding Begins at`,'RPL-20260815-PHD2-GUIDING-BEGIN','CIT-RPL-F3-PHD2-GUIDING-BEGIN')
];
const byId = Object.fromEntries(events.map(e=>[e.replay_event_id,e]));
function corr(id,left,right){
  const l=byId[left], r=byId[right];
  return {correlation_id:id,session_id:sessionId,left_event_ref:`replay-event:${left}`,right_event_ref:`replay-event:${right}`,relationship_type:'SEQUENTIAL',delta_ms:Date.parse(r.event_time_utc)-Date.parse(l.event_time_utc),classification_method_id:'BKL040-F3-EXACT-DELTA-1',classification_state:'NOT_ASSESSED',citation_refs:[...l.citation_refs,...r.citation_refs],provenance_refs:[...l.provenance_refs,...r.provenance_refs],conflict_refs:[]};
}
const correlations = [
  corr('CORR-F3-CW-NINA','RPL-20260815-CLOUDWATCHER-FIRST','RPL-20260815-NINA-DOME-OPEN'),
  corr('CORR-F3-NINA-PHD2','RPL-20260815-NINA-DOME-OPEN','RPL-20260815-PHD2-GUIDING-BEGIN')
];
