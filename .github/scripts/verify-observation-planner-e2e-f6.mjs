...
assert.ok(roadmap.projectStatus.includes('F3–F8 Accepted/Post-Merge Verified'), 'roadmap must preserve predecessor acceptance state');
assert.ok(roadmap.projectStatus.includes('F9 MeteoHub implementation'), 'roadmap must preserve the F9 implementation state');
assert.ok(roadmap.target.includes('No scheduler, automatic target selection, device command, readiness/go-no-go or Safety Authority is authorized'), 'F9 target must preserve authority boundaries');
