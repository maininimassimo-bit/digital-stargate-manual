...
for (const path of [projectionPath, schemaPath, architecturePath, portalPath, consumerPath, workflowPath, f5AcceptancePath]) {
...
for (const required of [
  ...
  'S10',
  'BKL-032',
  'forecast-refresh/runtime gate',
  'local physical interlocks'
]) assert.ok(architecture.includes(required), `architecture missing F6 boundary/evidence marker: ${required}`);
...
console.log('BKL-031 F6 verified: real bounded F4-C values + governed setup evidence + F5 method evidence, sanitized E2E portal path, zero provider requests, no readiness/command/Safety authority.');
