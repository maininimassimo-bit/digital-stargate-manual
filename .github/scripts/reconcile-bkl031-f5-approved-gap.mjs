...
function replaceLine(path, prefix, newLine) {
  ...
  const matches = lines.map((line, index) => [line, index]).filter(([line]) => line.startsWith(prefix));
  if (matches.length !== 1) throw new Error(`${path}: expected one line starting ${prefix}, found ${matches.length}`);
  lines[matches[0][1]] = newLine;
...
replaceOnce(
  ...
  'Factor values remain synthetic EVALUATION evidence only; provider budget remains 2/2 exhausted; F6 capability closure is next; S10 production runtime remains `UNAVAILABLE`.',
  'Factor values remain synthetic EVALUATION evidence only; provider budget remains 2/2 exhausted; F6 real-evidence setup-aware E2E planner integration is next; capability closure is deferred pending a separately governed forecast-refresh/runtime gate; S10 production runtime remains `UNAVAILABLE`.'
);
replaceOnce(
  'docs/project/index.md',
  'BKL-031 [F1-F5 ACCEPTED / F6 CLOSURE NEXT / S10 PRODUCTION UNAVAILABLE]',
  'BKL-031 [F1-F5 ACCEPTED / F6 REAL-EVIDENCE SETUP-AWARE E2E NEXT / CLOSURE DEFERRED / S10 PRODUCTION UNAVAILABLE]'
);

const f5Architecture = 'docs/architecture/scientific-assets/BKL-031-F5-Explainable-Ranking-Method-and-Read-Only-Consumer.md';
let architecture = read(f5Architecture);
const architectureMarker = '## 11. Successor boundary\n\n';
const ai = architecture.indexOf(architectureMarker);
if (ai < 0) throw new Error('F5 architecture successor section missing');
architecture = `${architecture.slice(0, ai)}${architectureMarker}With F5 Accepted/Post-Merge Verified, **F6 — real-evidence setup-aware E2E planner integration** becomes the next separately governed BKL-031 slice. F6 must bind the accepted F4-C real forecast evidence values, governed setup compatibility, accepted astronomical/lunar evidence and the F5 explainable ranking method into a deterministic read-only end-to-end planner proof and portal consumer. F6 performs no new provider request and must preserve the exhausted \`2/2_EXHAUSTED\` budget, no protected-site use, no readiness/go-no-go, no scheduling, no automatic target selection, no commands and no Safety Authority. BKL-031 capability closure is explicitly deferred until a later separately governed forecast-refresh/runtime gate proves how fresh forecast evidence is supplied without weakening ADR-011 lineage or the safety boundary.\n`;
write(f5Architecture, architecture);

const f5Acceptance = 'docs/project/BKL-031-F5-EXPLAINABLE-RANKING-ACCEPTANCE-2026-09-17.md';
let acceptance = read(f5Acceptance);
const acceptanceMarker = '## Transition\n\n';
const aci = acceptance.indexOf(acceptanceMarker);
if (aci < 0) throw new Error('F5 acceptance transition section missing');
acceptance = `${acceptance.slice(0, aci)}${acceptanceMarker}F5 acceptance authorizes only the separately governed **F6 real-evidence setup-aware E2E planner integration** package. F6 must consume the already accepted F4-C real forecast evidence, governed setup compatibility and accepted astronomical/lunar evidence through the F5 explainable method, then prove the resulting read-only planner path end to end on the portal. F6 performs no new provider request and does not authorize readiness/go-no-go, scheduling, automatic target selection, commands or Safety Authority. BKL-031 capability closure is deferred until a later separately governed forecast-refresh/runtime gate establishes fresh forecast supply while preserving ADR-011 lineage, the exhausted current request budget and S10/runtime boundaries.\n`;
write(f5Acceptance, acceptance);
...
for (const path of verifierFiles) {
  ...
  source = source.replaceAll("'BKL-031 F5 explainable ranking method and read-only consumer'", `NEXT`);
  source = source.replaceAll("'BKL-031 F6 capability closure'", `NEXT`);
  if (source.includes('NEXT') && !source.includes(`const NEXT='${NEXT}'`) && !source.includes(`const NEXT = '${NEXT}'`)) {
    const importEnd = source.lastIndexOf("import ");
    const firstBlank = source.indexOf('\n\n');
    if (firstBlank < 0) throw new Error(`${path}: cannot insert NEXT constant`);
    source = `${source.slice(0, firstBlank + 2)}const NEXT='${NEXT}';\n${source.slice(firstBlank + 2)}`;
  }
  source = source.replaceAll('F5 is the next separately governed slice.', 'F5 is accepted; F6 real-evidence setup-aware E2E planner integration is next.');
  source = source.replaceAll('F5 is the next separately governed slice.', 'F5 is accepted; F6 real-evidence setup-aware E2E planner integration is next.');
  source = source.replaceAll('F5 is the next separately governed slice', 'F5 is accepted; F6 real-evidence setup-aware E2E planner integration is next');
  source = source.replaceAll('F5 next.', 'F5 accepted; F6 real-evidence setup-aware E2E next.');
  source = source.replaceAll('F4 accepted and F5 next.', 'F4/F5 accepted; F6 real-evidence setup-aware E2E next.');
  source = source.replaceAll('F6 closure next.', 'F6 real-evidence setup-aware E2E planner integration next; closure deferred.');
  source = source.replaceAll('F5 accepted and F6 closure next.', 'F5 accepted; F6 real-evidence setup-aware E2E planner integration next; closure deferred.');
  write(path, source);
}
