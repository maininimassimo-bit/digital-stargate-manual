...
DEFAULT_FIELDS=['schema_version','session_id','session_start','session_end','target_name','ra_deg','dec_deg','coordinate_epoch','configuration_id','telescope','camera','binning','guide_profile','duration_hours','integration_hours','light_started','light_completed','light_failed','completion_pct','rms_ra_arcsec','rms_dec_arcsec','rms_total_arcsec','weather_safe_pct','sqm_state','sqm_start','sqm_end','sqm_min_mag_arcsec2','sqm_max_mag_arcsec2','sqm_mean_mag_arcsec2','sqm_median_mag_arcsec2','sqm_valid_samples','sqm_temporal_coverage','sqm_source','sqm_quality','autofocus_count','autofocus_failed','dither_count','dither_failed','severity','source_metrics_path','updated_at_utc']
def value(m,k):
...
def build_row(metrics,metrics_path):
    ...
    return {'schema_version':SCHEMA_VERSION,'session_id':metrics['session_id'],'session_start':'','session_end':'','target_name':value(s,'target_name'),'ra_deg':value(s,'ra_deg'),'dec_deg':value(s,'dec_deg'),'coordinate_epoch':value(s,'epoch'),'configuration_id':value(s,'configuration_id'),'telescope':value(s,'telescope'),'camera':value(s,'camera'),'binning':value(s,'binning'),'guide_profile':'','duration_hours':'','integration_hours':'' if seconds is None else round(float(seconds)/3600,4),'light_started':started,'light_completed':completed,'light_failed':failed,'completion_pct':round(completed/started*100,2) if started else '','rms_ra_arcsec':value(p,'rms_ra_arcsec'),'rms_dec_arcsec':value(p,'rms_dec_arcsec'),'rms_total_arcsec':value(p,'rms_total_arcsec'),'weather_safe_pct':'','sqm_state':value(q,'state'),'sqm_start':value(q,'start'),'sqm_end':value(q,'end'),'sqm_min_mag_arcsec2':value(q,'min_mag_arcsec2'),'sqm_max_mag_arcsec2':value(q,'max_mag_arcsec2'),'sqm_mean_mag_arcsec2':value(q,'mean_mag_arcsec2'),'sqm_median_mag_arcsec2':value(q,'median_mag_arcsec2'),'sqm_valid_samples':value(q,'valid_samples'),'sqm_temporal_coverage':value(q,'temporal_coverage'),'sqm_source':value(q,'source'),'sqm_quality':value(q,'quality'),'autofocus_count':int(n.get('autofocus_started') or 0),'autofocus_failed':int(n.get('autofocus_failed_explicit') or 0),'dither_count':int(n.get('dither_requests') or 0),'dither_failed':'','severity':value(metrics,'severity'),'source_metrics_path':Path(metrics_path).as_posix(),'updated_at_utc':datetime.now(timezone.utc).isoformat(timespec='seconds')}
def main():
