...
def test_real_c8_session():
    folder=ROOT/'data'/'sessions'/'2026'/'09'/'2026-09-13_2026-09-14'/'raw'/'phd2'
    parsed=MOD.parse_phd2(folder)
    assert parsed['equipment_profiles']==['C8_QHY695A'],parsed
    assert parsed['guide_segments']==7,parsed
    assert parsed['guide_samples_total']==6189,parsed
    assert parsed['guide_samples_valid']==5501,parsed
    assert parsed['guide_samples_saturated']==5501,parsed
    assert parsed['guide_samples_rejected']==346,parsed
    assert parsed['guide_samples_settling_excluded']==358,parsed
    assert parsed['settling_failures']==26,parsed
    assert_close(parsed['rms_ra_arcsec'],1.210)
