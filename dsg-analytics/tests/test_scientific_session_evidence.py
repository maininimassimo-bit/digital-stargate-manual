...
def main():
    with tempfile.TemporaryDirectory() as td:
        ...
        sqm=root/'sqm'; sqm.mkdir(); (sqm/'sqm-summary.json').write_text(json.dumps({'statistics':{'min':8.91,'max':20.42,'mean':17.5395,'median':18.66,'valid_samples':1301,'temporal_coverage':0.9849},'quality':{'state':'AVAILABLE'}}),encoding='utf-8'); projection=MOD.parse_sqm(sqm); assert projection['state']=='AVAILABLE'; assert projection['median_mag_arcsec2']==18.66; assert projection['valid_samples']==1301
