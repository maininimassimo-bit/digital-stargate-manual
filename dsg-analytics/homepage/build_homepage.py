...
def main():
    ...
    if latest:
        ...
        latest_rows = {
            ...
            "rms": (
                f"{dec(as_float(latest.get('rms_total_arcsec')), 3)} arcsec"
                if as_float(latest.get("rms_total_arcsec")) > 0
                else "—"
            ),
...
<section class="dsg-program-section">
  <div class="dsg-section-intro">
    <span class="dsg-section-kicker">PORTALE DINAMICO</span>
    <h2>Stato del programma e patrimonio documentale</h2>
  </div>
...
<div class="dsg-enterprise-meta">
  ...
  <span>Generato il {generated_at}</span>
  <span>ADR pubblicati: {adr_count}</span>
  <span>Prossima milestone: {esc(program['next_milestone'])}</span>
</div>
{END_MARKER}'''
