...
internal sealed class SqmTelemetryAdapter : IDisposable {
    ...
    private string lastPersistedObservedAtUtc;
    ...
    internal SqmTelemetryAdapter(SqmTelemetryOptions configuredOptions) {
        ...
        historyPath = Path.Combine(
            ...
            "DigitalStarGate", "telemetry", "sqm-history.ndjson");
    ...
    public Dictionary<string, object> Observe() {
        lock (stateLock) { return Clone(latest); }
    }
    ...
    private void PollLoop() {
        while (!stopEvent.WaitOne(0)) {
            try {
                ...
                var observation = Parse(payload, DateTime.UtcNow);
                SetLatest(observation);
                TryPersistCurrentSample(observation);
            } catch {
    ...
    private void TryPersistCurrentSample(Dictionary<string, object> observation) {
        try {
            if (!observation.TryGetValue("state", out var state) || !string.Equals(state as string, "AVAILABLE", StringComparison.Ordinal)) return;
            if (!observation.TryGetValue("details", out var detailsValue) || detailsValue is not Dictionary<string, object> details) return;
            if (!details.TryGetValue("quality", out var quality) || !string.Equals(quality as string, "CURRENT", StringComparison.Ordinal)) return;
            if (!details.TryGetValue("observedAtUtc", out var observedValue) || observedValue is not string observedAtUtc || string.IsNullOrWhiteSpace(observedAtUtc)) return;
            if (string.Equals(observedAtUtc, lastPersistedObservedAtUtc, StringComparison.Ordinal)) return;
            if (!details.TryGetValue("sqmMagArcsec2", out var sqmValue) || sqmValue is not double sqm || sqm <= 0) return;
            ...
            var sample = new Dictionary<string, object> {
                ...
                { "sqmMagArcsec2", sqm },
                { "source", details.TryGetValue("source", out var source) ? source : null },
                { "serial", details.TryGetValue("serial", out var serial) ? serial : null },
                { "firmware", details.TryGetValue("firmware", out var firmware) ? firmware : null },
                { "quality", "CURRENT" }
            };
            File.AppendAllText(historyPath, JsonSerializer.Serialize(sample, HistorySerializerOptions) + Environment.NewLine);
            lastPersistedObservedAtUtc = observedAtUtc;
        } catch {
            // History persistence is best-effort observability and must never degrade realtime telemetry.
        }
    }
    ...
    private Dictionary<string, object> Parse(string payload, DateTime receivedAtUtc) {
        ...
        if (!DateTime.TryParseExact(timestampText, "yyyy/MM/dd HH:mm:ss", CultureInfo.InvariantCulture,
        ...
        if (!double.TryParse(mpsasText, NumberStyles.Float, CultureInfo.InvariantCulture, out var mpsas) || double.IsNaN(mpsas) || double.IsInfinity(mpsas) || mpsas <= 0)
            return BuildUnknown("SQM_LIGHTMPSAS_INVALID", cwinfo, observedAtUtc, null, null);
        var age = receivedAtUtc - observedAtUtc;
        if (age < TimeSpan.FromSeconds(-5)) return BuildUnknown("SQM_SOURCE_TIMESTAMP_IN_FUTURE", cwinfo, observedAtUtc, null, null);
        ...
        if (receivedAtUtc > freshUntilUtc) return BuildService(true, "STALE", "SQM_SOURCE_STALE", new Dictionary<string, object> {
            { "source", "AAG CloudWatcher SOLO HTTP / lightmpsas" }, { "endpoint", endpoint.ToString() }, { "configurationSource", options.ConfigurationSource },
            { "sqmMagArcsec2", null }, { "observedAtUtc", observedAtUtc.ToString("o") }, { "freshUntilUtc", freshUntilUtc.ToString("o") },
            { "quality", "STALE" }, { "serial", serial }, { "firmware", firmware }
        });
        return BuildService(true, "AVAILABLE", null, new Dictionary<string, object> {
            { "source", "AAG CloudWatcher SOLO HTTP / lightmpsas" }, { "endpoint", endpoint.ToString() }, { "configurationSource", options.ConfigurationSource },
            { "sqmMagArcsec2", mpsas }, { "observedAtUtc", observedAtUtc.ToString("o") }, { "freshUntilUtc", freshUntilUtc.ToString("o") },
            { "quality", "CURRENT" }, { "serial", serial }, { "firmware", firmware }
        });
    }
    ...
    private Dictionary<string, object> BuildUnknown(string reason, string cwinfo, DateTime? observedAtUtc, string serial, string firmware) {
        ...
        return BuildService(null, "UNKNOWN", reason, new Dictionary<string, object> {
            { "source", "AAG CloudWatcher SOLO HTTP / lightmpsas" }, { "endpoint", endpoint.ToString() }, { "configurationSource", options.ConfigurationSource },
            { "sqmMagArcsec2", null }, { "observedAtUtc", observedAtUtc?.ToString("o") }, { "freshUntilUtc", null }, { "quality", "UNKNOWN" },
    ...
    private static string ParseCwInfoPart(string cwinfo, string prefix) {
        ...
        foreach (var part in cwinfo.Split(',')) {
            ...
            if (trimmed.StartsWith(prefix, StringComparison.OrdinalIgnoreCase)) return trimmed.Substring(prefix.Length).Trim();
        }
        return null;
    }
    ...
    private void SetLatest(Dictionary<string, object> value) { lock (stateLock) { latest = value; } }
    private static Dictionary<string, object> Clone(Dictionary<string, object> source) {
        var clone = new Dictionary<string, object>();
        foreach (var pair in source) clone[pair.Key] = pair.Value is Dictionary<string, object> nested ? new Dictionary<string, object>(nested) : pair.Value;
        return clone;
    }
}
