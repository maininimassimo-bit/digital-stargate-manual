...
| BKL-042 | P2 | AI Observatory Assistant | In Progress | BKL-015/BKL-044 e intelligence services | Copilot read-only per diagnosis/RCA/planning/science | F1-F5 bounded ACCEPTED (`52403dfb`); F6 OAuth acceptance preesistente; retrieval/provenance iniziale limitato a sessioni e identità target; normalizzazione query `M27`/`M 27` in PR #357 (`7be9fd0b`), method v2 deployato su relay `00007-wrm`, rollback `00006-ktv`; primo OAT `INSUFFICIENT_EVIDENCE` per mancato match prima del fix, nuova OAT owner-witnessed post-deploy e source coverage residua pending. Storage/upload, processing, tool, command e Safety Authority restano esclusi |
