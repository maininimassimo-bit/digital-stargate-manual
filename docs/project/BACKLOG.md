...
| BKL-046 | P2 | AI Post-Processing Assistant for PixInsight | In Progress | BKL-015/BKL-044/BKL-045 | Assistente advisory per ottimizzare workflow PixInsight | F1 current: source discovery and advisory semantic contract; human approval retained; no model/provider, apply path or implementation accepted |
...
F1 è corrente per inventariare le source eleggibili e definire il semantic boundary fra recommendation, decisione umana ed execution evidence. Riusa BKL-044/BKL-045, preserva `OBSERVED`/`DECLARED`/`SUGGESTED` e missingness fail-closed. Nessun modello, provider, RAG/vector store, parametro universale, image upload o PixInsight apply path è selezionato o autorizzato.
