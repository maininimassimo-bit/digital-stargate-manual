...
| BKL-031 | P1 | Observation Planner intelligente | In Progress | F3-C Accepted/Post-Merge Verified with 35/35 tests; F4-A and ADR-011 Accepted/Post-Merge Verified; F4-B Accepted/Post-Merge Verified at v1.0 with zero provider traffic; F4-C gate 001 merged and dispatched once, but the request was consumed with HTTP 400 because the Previous Runs host contradicted the Single Runs contract; no protected-site use; S10 unavailable | Accept F4-B v1.1 and F4-C gate 002, then dispatch exactly one replacement request and reconcile the artifact; no third request | ADR-010; ADR-011; `BKL-031-F4-C-FAILURE-001`; `BKL-031-F4-C-GATE-002`; run `35201479378` |
