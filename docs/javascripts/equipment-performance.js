(() => {
  ...
  fetch('../data/equipment-performance-registry-f5-collection.json',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('projection unavailable');return r.json();}).then(data=>{
    if(data.component!=='DSG.EquipmentPerformanceRegistry.F5.CollectionReadModel'||data.authority!=='projection'||data.action_authority!=='NONE'||!Array.isArray(data.views)||data.views.length!==data.population_count||!data.views.length||!Array.isArray(data.exclusions)) throw new Error('collection drift');
