(() => {
  ...
  const bytes = value => {
    ...
    if (n >= 1024 ** 3) return `${(n / (1024 ** 3)).toLocaleString('it-IT', { maximumFractionDigits: 1 })} GiB`;
    if (n >= 1024 ** 2) return `${(n / (1024 ** 2)).toLocaleString('it-IT', { maximumFractionDigits: 1 })} MiB`;
  ...
  const render = (payload, transport = 'HOSTED') => {
    ...
    set('transport', transport); set('quality', badge(overallQuality));
    set('observed-at', parseTime(payload?.observed_at_utc)?.toLocaleString('it-IT') || '—'); set('source', text(payload?.source_component));
  ...
  const renderEagleUnavailable = quality => {
    setEagle('quality', badge(quality || 'UNKNOWN')); setEagle('summary', 'UNKNOWN / POLICY_NOT_ACTIVATED'); setEagle('observed-at', '—'); setEagle('source', '—');
    ['cpu','memory','storage-capacity','storage-health','uptime','time-sync'].forEach(key => setEagle(key, '—'));
  };

  const renderEagle = payload => {
    ...
    const payloadFresh = isFresh(payload), signals = payload?.signals || {};
    setEagle('quality', badge(payloadFresh ? payload.quality : 'STALE'));
    setEagle('summary', 'UNKNOWN / POLICY_NOT_ACTIVATED');
    setEagle('observed-at', parseTime(payload?.observed_at_utc)?.toLocaleString('it-IT') || '—');
    setEagle('source', `${text(payload?.source_component)} · ${text(payload?.host)}`);
    ...
    const cpuData = cpu.quality === 'CURRENT' ? cpu.data || {} : {};
    const memoryData = memory.quality === 'CURRENT' ? memory.data || {} : {};
    const storageData = storage.quality === 'CURRENT' ? storage.data || {} : {};
    const uptimeData = uptime.quality === 'CURRENT' ? uptime.data || {} : {};
    const timeData = timeSync.quality === 'CURRENT' ? timeSync.data || {} : {};
    ...
    const logical = Array.isArray(storageData.logical_disks) ? storageData.logical_disks : [];
    setEagle('storage-capacity', storage.quality === 'CURRENT' && logical.length ? logical.map(d => `${text(d.device_id)} ${bytes(d.free_bytes)} liberi / ${bytes(d.size_bytes)} (${number(d.free_pct, 1, ' %')})`).join(' · ') : `${text(storage.quality)} · ${text(storage.reason)}`);
    const physical = Array.isArray(storageData.physical_disks) ? storageData.physical_disks : [];
    setEagle('storage-health', storage.quality === 'CURRENT' && physical.length ? physical.map(d => `${text(d.friendly_name)}: ${text(d.health_status)} / ${Array.isArray(d.operational_status) ? d.operational_status.join(', ') : text(d.operational_status)}`).join(' · ') : `${text(storage.quality)} · ${text(storage.reason)}`);
  ...
  const refreshEagleHealth = async () => { try { renderEagle(await fetchJson(new URL(EAGLE_HEALTH_PATH, document.baseURI))); } catch (error) { console.warn('Digital StarGate: EAGLE health projection unavailable', error); renderEagleUnavailable('UNKNOWN'); } };

  let timer;
  const refresh = async () => {
    try { render(await fetchJson(RUNTIME_ENDPOINT), 'CLOUD RUN'); return; } catch (runtimeError) { console.warn('Digital StarGate: hosted observatory telemetry unavailable', runtimeError); }
    try { render(await fetchJson(new URL(FALLBACK_DATA_PATH, document.baseURI)), 'FALLBACK'); } catch (fallbackError) { console.warn('Digital StarGate: fallback observatory telemetry unavailable', fallbackError); renderUnavailable(); }
  };

  const initialize = () => {
    if (!document.querySelector('[data-observatory-status], [data-eagle-health]')) return;
    clearInterval(timer); refreshLatestScientific(); refreshEagleHealth(); refresh();
    timer = window.setInterval(() => { refresh(); refreshEagleHealth(); }, REFRESH_MS);
  };

  if (window.DSG?.components) window.DSG.components.register({ name: 'observatory-status', order: 66, initialize });
  else if (window.document$?.subscribe) window.document$.subscribe(initialize);
