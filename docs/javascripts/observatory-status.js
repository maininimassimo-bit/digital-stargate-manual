(() => {
  ...
  const renderEagle = (payload,transport='HOSTED') => {
    if(payload?.component!=='DSG.EagleHealthPortalProjection'||!['HEALTHY','DEGRADED','UNAVAILABLE'].includes(payload?.summary?.state)||!payload?.summary?.reason)throw new Error('Invalid EAGLE health portal projection contract');
