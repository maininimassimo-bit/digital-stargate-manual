...
The next reviewed source gate publishes only the already reproduced OCI manifest `sha256:de3331882e767c3a16fc479224da7c540385a6460e26df1ac73f8305676a0cce`. It is explicit-dispatch and main-only, verifies the exact empty registry, performs two independent OCI rebuilds plus a no-push registry-exporter preflight, and has one guarded push exporter. It then verifies the raw registry manifest digest, config digest and exclusive one-image inventory. This source package does not dispatch the workflow. The authenticated four-resource plan remains a later gate, and platform apply remains blocked.
