...
The GitHub workflow may publish only the small read-only JSON projection. Standard runners in the public repository are the execution boundary; EAGLE is not involved.
...
The subscription cost is eliminated, but F9 assumes operational parsing of official GRIB data and availability of MeteoHub. Provider failure never revives stale F7/F8 data as current. Attribution to ItaliaMeteo/ARPAE and the CC BY 4.0 source must remain visible.
