...
| Local receipt outbox / delayed delivery | On 2026-09-26 Massimo Mainini selected preserving locally produced receipts through network interruption and sending them after reconnection; he also selected survival of source reboot and unexpected power loss, preserving all unacknowledged entries while explicitly signalling a full queue rather than evicting them, and applying both byte-capacity and queued-age limits. For age evaluation, use source-observed time when clock quality is reliable and mark age uncertain otherwise; pending receipts are not deleted. This only covers observations created while the source operates and does not evidence activity while EAGLE is off. Power-loss durability and full-queue signalling are requirements to verify, not proven guarantees. | **OWNER DESIGN DIRECTION RECORDED; numeric byte/age thresholds, clock-quality criteria, admission behavior under uncertain age, persistence mechanism/flush semantics, full-condition persistence/visibility, disk-full/corruption recovery, access controls, retry/ack, late-receipt reconciliation and local installation authorization pending** |
| Sampling/heartbeat cadence, timeout, freshness and gap semantics | On 2026-09-26 Massimo Mainini selected one receipt per minute and an approximately two-minute threshold for flagging a candidate interval without receipts in an offline report. This does not identify cause or authorize live gap detection/alerting. Historical producer cadence is not transferable; clock/delivery uncertainty, timeout, freshness, retry and exact `UNKNOWN` interval boundaries remain unresolved. | **ONE-MINUTE CADENCE AND APPROXIMATE TWO-MINUTE OFFLINE GAP THRESHOLD RECORDED; exact contract and runtime authorization pending** |
...
| Local/remote data stores | On 2026-09-26 Massimo Mainini selected single-region receipt storage and deferred the specific region until a cost, latency and data-residency comparison. Cloud Storage remains a candidate; no project or concrete region is selected and no EU-only constraint is assumed. | **DESIGN PREFERENCES RECORDED; comparison, project, region, IAM, retention, backup/recovery and runtime approval pending** |
...
| Cost ceiling and billing controls | On 2026-09-26 Massimo Mainini chose to set the monthly ceiling after a configuration-specific cost comparison. Preliminary scenarios are recorded in `BKL-043-F4-COST-ESTIMATE-2026-09-25.md`; account plan, remaining quota, billing owner, allowed products, one-time ceiling and final region are not yet known. | **MONTHLY CEILING DEFERRED UNTIL UPDATED ESTIMATE; all billing controls remain unselected; no spending or resource creation authorized** |
