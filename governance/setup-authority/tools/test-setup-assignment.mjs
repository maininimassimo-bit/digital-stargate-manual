...
import {
  ...
  validateApprovalReceipt, validatePromotion, validateBinding, assertSchemaValidatorParity, intervalContains, intervalsOverlap,
...
test("D4-P05 schemas match validator closed-key coverage",async()=>{const [a,d,r]=await Promise.all([real("governance/setup-authority/schemas/current-setup-assignment.schema.json"),real("governance/setup-authority/schemas/setup-assignment-owner-decision.schema.json"),real("governance/setup-authority/schemas/current-setup-assignment-approval-receipt.schema.json")]);assert.equal(assertSchemaValidatorParity(a,d,r).valid,true);});
...
test("D5-P01 protected approval receipt validates closed",async()=>assert.equal(validateApprovalReceipt(await real("governance/setup-authority/approval-evidence/DSG-CURRENT-SETUP-ASSIGNMENT-001.approval.json")).valid,true));
test("D5-P02 APPROVED repository envelope validates",async()=>assert.equal(validateAssignment(await real("governance/setup-authority/setup-assignments/DSG-CURRENT-SETUP-ASSIGNMENT-001.approved.json")).valid,true));
test("D5-P03 approval preserves exact canonical payload and digest",async()=>{const [d,a]=await Promise.all([real("governance/setup-authority/setup-assignments/DSG-CURRENT-SETUP-ASSIGNMENT-001.draft.json"),real("governance/setup-authority/setup-assignments/DSG-CURRENT-SETUP-ASSIGNMENT-001.approved.json")]);assert.equal(canonicalize(d.assignmentPayload),canonicalize(a.assignmentPayload));assert.equal(d.payloadDigest.value,a.payloadDigest.value);});
test("D5-P04 receipt and lifecycle promotion bind exactly",async()=>{const [d,a,r]=await Promise.all([real("governance/setup-authority/setup-assignments/DSG-CURRENT-SETUP-ASSIGNMENT-001.draft.json"),real("governance/setup-authority/setup-assignments/DSG-CURRENT-SETUP-ASSIGNMENT-001.approved.json"),real("governance/setup-authority/approval-evidence/DSG-CURRENT-SETUP-ASSIGNMENT-001.approval.json")]);assert.deepEqual(validatePromotion(d,a,r).errors,[]);});
test("D5-P05 approved assignment resolves only through approved sources",async()=>{const [a,site,baseline]=await Promise.all([real("governance/setup-authority/setup-assignments/DSG-CURRENT-SETUP-ASSIGNMENT-001.approved.json"),real("governance/site-authority/site-records/DSG-SITE-RECORD-MANCIANO-001.approved.json"),real("governance/setup-authority/configuration-baselines/DSG-SETUP-BASELINE-001.approved.json")]);assert.equal(resolveCurrentSetup([a],{observatoryId:a.assignmentPayload.observatoryId,asOfUtc:a.assignmentPayload.validity.validFromUtc,sources:{siteRecords:[site],baselines:[baseline]}}).state,"AVAILABLE");});
test("D5-N01 payload mutation during promotion fails closed",async()=>{const [d,a,r]=await Promise.all([real("governance/setup-authority/setup-assignments/DSG-CURRENT-SETUP-ASSIGNMENT-001.draft.json"),real("governance/setup-authority/setup-assignments/DSG-CURRENT-SETUP-ASSIGNMENT-001.approved.json"),real("governance/setup-authority/approval-evidence/DSG-CURRENT-SETUP-ASSIGNMENT-001.approval.json")]);a.assignmentPayload.revision+=1;assert.ok(validatePromotion(d,a,r).errors.includes("PAYLOAD_MUTATED_DURING_APPROVAL"));});
test("D5-N02 custodian or non-owner approval fails closed",async()=>{const r=await real("governance/setup-authority/approval-evidence/DSG-CURRENT-SETUP-ASSIGNMENT-001.approval.json");r.approvingAuthority.authorityRef="role:digital-stargate-architecture-office";r.approvingAuthority.custodianIsApprover=true;assert.ok(validateApprovalReceipt(r).errors.includes("INVALID_APPROVING_AUTHORITY"));});
test("D5-N03 receipt subject or validity mismatch fails binding",async()=>{const [d,a,r]=await Promise.all([real("governance/setup-authority/setup-assignments/DSG-CURRENT-SETUP-ASSIGNMENT-001.draft.json"),real("governance/setup-authority/setup-assignments/DSG-CURRENT-SETUP-ASSIGNMENT-001.approved.json"),real("governance/setup-authority/approval-evidence/DSG-CURRENT-SETUP-ASSIGNMENT-001.approval.json")]);r.subject.validFromUtc="2026-09-15T00:00:00.000Z";assert.ok(validatePromotion(d,a,r).errors.includes("APPROVAL_SUBJECT_BINDING_FAILED"));});
