...
The first protected candidate remains `DRAFT` until the Repository Owner is presented with and approves its exact canonical payload digest. The candidate does not make S08 available and does not create a `CurrentSetupAssignment`.
...
- `approval-evidence/`: future immutable exact-digest lifecycle approvals;
...
The validator checks lifecycle, authority, WGS84 ranges, orthometric elevation semantics, IANA timezone, half-open validity, digest integrity and absence of protected literals from `docs/`. Its output intentionally omits protected values and internal digests.
